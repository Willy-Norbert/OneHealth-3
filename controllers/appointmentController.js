...
const { sendAppointmentEmail, sendDoctorBookingEmail } = require('../services/emailService');
const { FEATURE_DOCTOR_ADVANCED_NOTIFICATIONS } = require('../config/featureFlags');
...
console.log('🗂 Active SlotLocks for hospital/department:', 
  await SlotLock.find({ hospital, department, expiresAt: { $gt: new Date() } })
    ...
    // Step 2a: Enforce doctor availability (if doctor specified)
    if (doctor) {
      try {
        const docProfile = await (require('../models/Doctor')).findOne({ $or: [{ _id: doctor }, { user: doctor }] }).select('settings');
        if (docProfile?.settings?.defaultAvailability) {
          const day = new Date(appointmentDate).toLocaleDateString('en-US', { weekday: 'long' });
          const ex = (docProfile.settings.defaultAvailability.exceptions || []).find((e) => new Date(e.date).toDateString() === new Date(appointmentDate).toDateString());
          const explicitlyUnavailable = ex && ex.available === false;
          const allowedWeekday = (docProfile.settings.defaultAvailability.weekdays || []).includes(day);
          if (explicitlyUnavailable || (!allowedWeekday && (docProfile.settings.defaultAvailability.weekdays || []).length > 0)) {
            return res.status(409).json({ status: 'error', message: 'Doctor is unavailable on the selected date' });
          }
        }
      } catch {}
    }

    // Step 2b: Check if slot is already booked (existing appointments)
...
console.log("👤 Authenticated user:", req.user);
    ...
    // Step 8: Send patient confirmation email
    try { 
    ...
    // Doctor notifications behind feature flag and respecting doctor settings
    if (FEATURE_DOCTOR_ADVANCED_NOTIFICATIONS && appointment.doctor) {
      try {
        const doctorUserId = appointment.doctor._id || appointment.doctor; 
        const doctorProfile = await Doctor.findOne({ user: doctorUserId }).select('settings').populate('user','email name');
        const doctorUser = appointment.doctor.user ? appointment.doctor.user : (doctorProfile ? { email: doctorProfile?.user?.email, name: doctorProfile?.user?.name } : null);
        const prefs = doctorProfile?.settings || { emailNotifications: true, inAppNotifications: true };
        ...
        if (prefs.inAppNotifications) {
          await createNotification({
            ...
            sender: req.user._id,
            type: 'appointment',
            message: `New appointment booked by ${appointment?.patientDetails?.fullName || 'a patient'} on ${appointment.appointmentDate.toDateString()} at ${appointment.appointmentTime}.`,
            relatedEntity: { id: appointment._id, type: 'Appointment' },
          });
        }
        if (prefs.emailNotifications && doctorUser) {
          await sendDoctorBookingEmail(appointment, doctorProfile?.user || doctorUser);
        }
      } catch (e) {
        console.warn('Doctor notification failed:', e.message);
      }
    }
...
console.log('🔍 GET ALL APPOINTMENTS - User:', req.user.role, req.user._id);
    ...
    if (req.user.role === 'hospital') {
      let hospitalId = req.user.hospital;
      if (!hospitalId) {
      ...
      console.log('🏥 Hospital filter applied:', hospitalId);
    ...
    // Fetch appointments with population including patient/doctor images and payment/meeting
    const appointments = await Appointment.find(query)
      .populate([
        { path: 'patient', select: 'fullName name email phoneNumber profileImage' },
        { path: 'doctor', select: 'fullName name profileImage' },
        { path: 'hospital', select: 'name location' },
        { path: 'payment' },
        { path: 'meeting' }
      ])
...
exports.getDoctorAppointments = async (req, res) => {
  try {
    ...
    if (from || to) {
      ...
      if (Object.keys(range).length) query.appointmentDate = range;
    }
