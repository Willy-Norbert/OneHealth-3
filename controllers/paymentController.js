...
// Manual confirmation by hospital role
const manualConfirmSchema = Joi.object({ notes: Joi.string().allow('') })
exports.manualConfirm = async (req, res) => {
  try {
    if (req.user.role !== 'hospital' && req.user.role !== 'admin') {
      return res.status(403).json({ status: 'error', message: 'Forbidden' })
    }
    const { error } = manualConfirmSchema.validate(req.body || {})
    if (error) return res.status(400).json({ status: 'error', message: error.message })
    ...
    return res.status(200).json({ status: 'success', data: { payment }, message: 'Payment manually confirmed' })
  } catch (err) {
    return res.status(500).json({ status: 'error', message: err.message })
  }
}