...
export default function DoctorLabResultsPage() {
  ...
  const { data: results, isLoading, error } = useSWR(['doctor-lab-results', params], () => api.labResults.list(params) as any)
                  ...
                  {results.data.docs.map((r:any)=>(
                    <div key={r._id} className="p-3 rounded-lg border flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-sm text-gray-600">{r.patient?.user?.name || r.patient?.user?.email}</div>
                      </div>
                      <div className="flex items-center gap-2">
                        ...
                        <Link href={`/doctor/lab-results/${r._id}`} className="btn-outline btn-sm">View</Link>
                      </div>
                    </div>
                  ))}
                </div>
