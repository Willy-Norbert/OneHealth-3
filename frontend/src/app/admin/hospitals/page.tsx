...
export default function AdminHospitalsPage() {
  ...
  async function apiFetch(path: string, init?: RequestInit) {
    ...
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api'}${path}`, { ...init, headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` } })
    return res.json()
  }
