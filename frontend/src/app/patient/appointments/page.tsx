...
export default function AppointmentsPage() {
  ...
  const loadDoctors = async (hospitalId: string, departmentId: string, date?: string) => {
    if (!hospitalId || !departmentId) return
    try {
      const params = new URLSearchParams()
      params.append('hospital', hospitalId)
      params.append('department', departmentId)
      if (date) params.append('availableOn', date)
      const res = await api.doctors.list(params) as any
              ...
              <form onSubmit={submit} className="space-y-6">
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                         ...
                         <div className="form-group">
                           ...
                           <select
                             ...
                             onChange={e => { 
                               const next = { ...form, department: e.target.value, doctor: '' };
                               setForm(next);
                               loadDoctors(next.hospital, e.target.value, next.appointmentDate);
                             }}
                             required
                           >
                  ...
                  <div className="form-group">
                    ...
                    <input 
                      ...
                      onChange={e => { const next = { ...form, appointmentDate: e.target.value }; setForm(next); if (next.hospital && next.department) loadDoctors(next.hospital, next.department, next.appointmentDate); }} 
                      required 
                    />
                  </div>
...
function PayButton({ appointmentId }: { appointmentId: string }) {
  ...
  return <button onClick={checkout} disabled={loading} className="btn-primary">{status || (loading? <HealthSpinner /> : 'Pay now')}</button>
}
