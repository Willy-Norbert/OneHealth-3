...
export default function AIPage() {
  ...
  const fetchUserName = async (): Promise<string> => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://onehealthconnect-nu4v.onrender.com'}/auth/me`, {
        credentials: 'include',
  ...
  const run = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isGreeting(symptoms)) {
        ...
        try {
          const base = process.env.NEXT_PUBLIC_API_URL || 'https://onehealthconnect-nu4v.onrender.com'
