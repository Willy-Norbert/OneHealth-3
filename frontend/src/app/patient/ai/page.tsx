...
export default function AIPage() {
  ...
  const fetchUserName = async (): Promise<string> => {
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://jk4k84k0so8g4ggg4oow4kcs.69.62.122.202.sslip.io'}/auth/me`, {
        credentials: 'include',
  ...
  const run = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isGreeting(symptoms)) {
        ...
        try {
          const base = process.env.NEXT_PUBLIC_API_URL || 'http://jk4k84k0so8g4ggg4oow4kcs.69.62.122.202.sslip.io'
