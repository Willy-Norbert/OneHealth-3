...
export default function HospitalDoctorEditPage() {
  ...
  const { data } = useSWR(() => (id ? `doctor-${id}` : null), async () => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/doctors/${id}`, { headers: { 'Content-Type': 'application/json' }})
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/doctors/${id}/hospital-update`, {
        method:'PUT',
