...
export default function HospitalDoctorEditPage() {
  ...
  const { data } = useSWR(() => (id ? `doctor-${id}` : null), async () => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://onehealthconnect-nu4v.onrender.com'}/doctors/${id}`, { headers: { 'Content-Type': 'application/json' }})
  ...
  async function save() {
    ...
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'https://onehealthconnect-nu4v.onrender.com'}/doctors/${id}/hospital-update`, {
        method:'PUT',
